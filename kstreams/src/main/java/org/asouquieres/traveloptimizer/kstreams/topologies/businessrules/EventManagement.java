package org.asouquieres.traveloptimizer.kstreams.topologies.businessrules;
...
import org.asouquieres.traveloptimizer.kstreams.topologies.models.Utils;
...
public class EventManagement {
    ...
    public static List<TravelAlert> whenACustomerRequestArose(CustomerTravelRequest request,
        ...
        if (optimalTravel != null) {
            Utils.updateAlert(newTravelAlert, optimalTravel, null);

            requestState.put(Utils.getRequestStateKey(optimalTravel.getTravelId(), request.getId()), newTravelAlert);

            // Forward the result
            result.add(newTravelAlert);
        } else {
            earlyRequestState.put(Utils.getEarlyRequestStateKey(departureLocation, arrivalLocation, request.getId()), request);
        }
    ...
    private static List<TravelAlert> handleImpactedActiveTravels(
        ...
        String stateStoreSearchPrefix = Utils.getStateStoreSearchPrefix(departureLocation, arrivalLocation);
        ...
        try (KeyValueIterator<String, TravelAlert> requestAlerts = requestState.prefixScan(stateStoreSearchPrefix, PREFIX_SERDE.serializer())) {
            while (requestAlerts.hasNext()) {
                ...
                String currentRequestAlertKey = Utils.getRequestStateKey(travelAlert.getTravelId(), travelAlert.getId());
                requestState.delete(currentRequestAlertKey);

                Utils.updateAlert(travelAlert, optimal, timeUpdate.getUpdateId());

                // Update Stores
                String newRequestAlertKey = Utils.getRequestStateKey(travelAlert.getTravelId(), travelAlert.getId());
    ...
    private static List<TravelAlert> handleEarlyRequests(KeyValueStore<String, TravelAlert> requestState, KeyValueStore<String, CustomerTravelRequest> earlyRequestState, TimeTableEntry timeUpdate) {
        ...
        try (KeyValueIterator<String, CustomerTravelRequest> iterator = earlyRequestState.prefixScan(stateStoreSearchPrefix, PREFIX_SERDE.serializer())) {
            while (iterator.hasNext()) {
                ...
                var newAlert = Utils.updateAlert(TravelAlert.fromRequest(next.value), timeUpdate, timeUpdate.getUpdateId());
                newAlert.setUpdateId(timeUpdate.getUpdateId());

                String newRequestAlertKey = Utils.getRequestStateKey(newAlert.getTravelId(), newAlert.getId());
    ...
    private static TimeTableEntry getOptimal(String departure,
                                             ...
                                             KeyValueStore<String, TimeTableEntry> timeTableState) {
        // Lookup for matching timetable entry using partition key 'prefix scan'
        String stateStoreSearchPrefix = Utils.getStateStoreSearchPrefix(departure, arrival);
